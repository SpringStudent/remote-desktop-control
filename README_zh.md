...
6. **局域网 P2P 直连**
    ...
    * 两端同处一个局域网时，屏幕画面和键鼠事件不经服务器中继，直接点对点传输 ，降低延迟、节省服务器带宽
...
┌──────────────┐    Netty TCP      ┌─┴──────────┴─┐     Netty TCP      ┌──────────────┐
│    控制端     │ ◄───────────────► │    Server    │ ◄───────────────► │    被控端     │
│  Controller  │    信令 + 中继     │   中转服务器   │    信令 + 中继     │  Controlled  │
│              │                   │              │                   │              │
│ • 解压渲染   │                   │ • 设备注册    │                   │ • 屏幕捕获   │
│ • 键鼠发送   │                   │ • 信令路由    │                   │ • 压缩编码   │
│ • P2P连接    │                   │ • Channel配对 │                   │ • Robot执行  │
│              │                   │              │                   │ • P2P监听    │
└──────┬───────┘                   └──────────────┘                   └──────┬───────┘
       │                                                                    │
       └─────────────────── P2P 直连 (局域网) ──────────────────────────────┘
                屏幕数据 / 键鼠指令 / 剪贴板文本 — 失败回退中继
                                                                    │
                                                                    │ Socket
                                                                    ▼
                                                             ┌──────────────┐
  ...
  Robots 仅在 Windows 锁屏时由被控端通过 Socket 调用。
  剪贴板: 文本走 Netty (中继或 P2P)；文件走 HTTP (分块上传/下载通过
  FileController，Netty 仅传递文件 ID 通知对方)。
```
